...
func TestDatabaseCreatesFileAndCRUD(t *testing.T) {
	...
	list, err := db.ListArticles(false, nil, nil, false)
	if err != nil {
...
func TestListArticlesFiltersAndOrdering(t *testing.T) {
	...
	all, err := db.ListArticles(false, nil, nil, false)
	if err != nil {
		t.Fatalf("list articles: %v", err)
	}
	if len(all) != 3 {
	...
	unread, err := db.ListArticles(true, nil, nil, false)
	if err != nil {
	...
	blogID := blogB.ID
	filtered, err := db.ListArticles(false, &blogID, nil, false)
	if err != nil {
...
func TestBulkInsertDuplicateRollbackAndEmpty(t *testing.T) {
	...
	articles, err := db.ListArticles(false, nil, nil, false)
	if err != nil {
		t.Fatalf("list articles: %v", err)
	}
	if len(articles) != 1 {
